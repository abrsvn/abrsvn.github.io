...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            break
        if re.search("^RULE FIRED", str(agreement_sim.current_event.action)):
            rules_fired.append(re.split(": ", str(agreement_sim.current_event.action))[1])
        #if agreement.retrieval and agreement.retrieval.copy().pop() not in retrieval_time_points:
            #retrieval_time_points[agreement.retrieval.copy().pop()] = agreement_sim.show_time()
    print("Rules fired")
    print(rules_fired)
    #print("Retrieval time points")
    #print(retrieval_time_points)
